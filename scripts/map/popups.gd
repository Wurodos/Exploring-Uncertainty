...
const _regime_change_prefab = preload("res://prefabs/map/regime_change_popup.tscn")
...
func _ready() -> void:
	...
	SignalBus.regime_change.connect(_on_regime_change)
...
func _make_popup(prefab: PackedScene) -> Node:
...
func _on_regime_change() -> void:
	_make_popup(_regime_change_prefab)

func _on_message_popup(string_id: StringName):
	var popup = _make_popup(_message_prefab)
	popup.apply(string_id)

func _on_found_item():
	_make_popup(_found_item_prefab)

func _on_lost_item(item: Item):
	var popup = _make_popup(_lost_item_prefab)
	popup.apply(item)
